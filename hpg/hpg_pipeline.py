#!/usr/bin/env python3
"""
HiPerGator segmentation pipeline -- one shard of one taxon, SAM3 text-prompted
instance segmentation.

Ported from chtc/staging_pipeline.py.  The differences that matter:

  * --shard / --num-shards.  Row i of the taxon belongs to shard
    (i % num_shards), so N SLURM array tasks split the taxon N ways with no
    coordination and no pre-partitioning step.  Same scheme as
    parallel_A100/segmentation_pipeline.py.
  * Output goes straight to a shared /blue path.  HiPerGator compute nodes
    mount the same Lustre filesystem as the login nodes, so there is no
    HTCondor-style transfer_input_files / output_destination dance -- every
    shard writes into ONE output tree and the results are already merged on
    disk when the array finishes.
  * Per-shard results CSV (results_shard_NNN.csv) and error log.  Shards never
    write the same file, which is what makes concurrent Lustre writes safe.
    Resume is per shard: rerun the same array and each task skips the photo_ids
    already in its own CSV.  Keep --num-shards constant across reruns.
  * --workers for the download pool (HPG nodes have far more cores per GPU than
    CHTC gave us) and --amp for bfloat16 autocast inference.

Usage (inside the container; normally invoked by hpg/run_shard.sh):
    python hpg_pipeline.py \
        --taxon-id 160559 \
        --output-dir /blue/<group>/<user>/results/160559_flower \
        --parquet /blue/<group>/<user>/data/inat_photos.parquet \
        --prompt "flower" \
        --shard 0 --num-shards 60 --workers 8 --amp
"""
...
_p.add_argument("--output-dir", required=True,            help="Output tree (absolute /blue path)")
_p.add_argument("--parquet",    type=str, required=True,
                help="Path to inat_photos.parquet on /blue (read-only, shared by all shards)")
...
_p.add_argument("--shard",      type=int, default=0,  help="This task's shard index (0-based)")
_p.add_argument("--num-shards", type=int, default=1,  help="Total shards in the array")
_p.add_argument("--workers",    type=int, default=8,  help="Image download threads")
_p.add_argument("--amp",        action="store_true",
                help="Run inference under bfloat16 autocast (faster on L4/B200)")
_args = _p.parse_args()
...
SHARD       = _args.shard
NUM_SHARDS  = max(1, _args.num_shards)
USE_AMP     = _args.amp

# --output-dir is normally an absolute /blue path; Path() handles both.
BASE_DIR   = Path(_args.output_dir).expanduser().resolve()
PARQUET    = Path(_args.parquet).expanduser()

IMAGE_BASE   = BASE_DIR / "images"
MASK_BASE    = BASE_DIR / "masks"
OVERLAY_BASE = BASE_DIR / "overlays"
SEGMENT_BASE = BASE_DIR / "segments"
# Per-shard files: concurrent shards must never append to the same file.
OUT_CSV      = BASE_DIR / f"results_shard_{SHARD:03d}.csv"
ERR_LOG      = BASE_DIR / f"errors_shard_{SHARD:03d}.txt"

BATCH_SIZE   = 1_000                    # images per batch folder
MAX_WORKERS  = max(1, _args.workers)
TIMEOUT_SEC  = 60
...
def build_df():
    ...
    # Interleaved sharding.  global_index is assigned over the FULL taxon first,
    # so batch_NNNNN folders and filenames are identical no matter how many
    # shards run -- changing --num-shards reshuffles who does what, not where
    # the output lands.
    if NUM_SHARDS > 1:
        total = len(raw)
        raw = raw[raw["global_index"] % NUM_SHARDS == SHARD % NUM_SHARDS].copy()
        print(f"Shard {SHARD}/{NUM_SHARDS}: {len(raw):,} of {total:,} photos")
        if raw.empty:
            raise SystemExit(f"Shard {SHARD} has no rows (taxon smaller than --num-shards).")
...
CSV_HEADER = [
    ...
    "global_index", "shard", "batch_name", "status", "num_masks",
...
def run_segmentation():
    ...
    print(f"SLURM job   : {os.environ.get('SLURM_JOB_ID', '-')}"
          f"  array task {os.environ.get('SLURM_ARRAY_TASK_ID', '-')}")
    df = build_df()
    print(f"\nTaxon ID    : {TAXON_ID}")
    ...
    print(f"Autocast    : {'bf16' if USE_AMP else 'off'}")
        ...
        while inflight:
            ...
            def meta_prefix(status, num, orel, mrels, srels, scores_str, err):
                return [pid, tid, tname, ext, qg, lat, lon, url,
                        gidx, SHARD, bn, status, num, irel, orel,
